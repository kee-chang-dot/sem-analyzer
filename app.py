...
with col_logo:
    ...
    if os.path.exists(logo_path):
        st.image(logo_path, use_column_width=True)  # 自动缩放适应列宽

with col_title:
    # 使用 HTML 和内联 CSS 让文字垂直居中
    st.markdown(
        """
...
st.markdown("---")  # 画一条灰色的横线分隔头部和下方的主体内容
# ==========================================
...
if uploaded_file is not None:
    # 只在刚上传时读取一次图像
    if st.session_state.img_matrix is None:
    ...
    with col_controls:
        ...
        # 【修改点1】调整了列表中选项的排列顺序，把标尺和区域放到了前面
        mode = st.radio("选择鼠标操作:",
                        ["设置标尺 (绿)", "框选区域 (黄)", "添加孔洞 (红)", "添加聚合物 (蓝)"])

        # 【修改点2】因为默认选中的变成了“设置标尺”，所以初始模式也要对应改成 line 和 green
        drawing_mode = "line"
        stroke_color = "green"
        ...
        if st.button("计算孔隙率", type="primary", use_container_width=True):
            with st.spinner("正在计算..."):
                ...
                if porosity is not None:
                    ...
                    st.pyplot(fig)  # 在网页上显示 Matplotlib 图表
                else:
                    st.error(f"计算失败: {msg}")
    ...
    with col_img:
        # 使用 st_canvas 替换原有的 self.fig.canvas.mpl_connect 交互
...
# 添加：页面底部（版权信息）
# ==========================================
st.markdown("<br><br><br>", unsafe_allow_html=True)  # 留出一些空白间距
